...
func TestParseArgs(t *testing.T) {
	cases := []struct {
		...
		{"attn bare", []string{"--attn"}, ParsedArgs{Attn: true}, false},
		{"attn with flags", []string{"--attn", "--kind", "opencode", "--event", "session.idle", "--cwd", "/g/x"},
			ParsedArgs{Attn: true, AttnKind: "opencode", AttnEvent: "session.idle", AttnCwd: "/g/x"}, false},
		{"attn equals forms", []string{"--attn", "--event=permission.updated", "--cwd=/g/y", "--kind=opencode"},
			ParsedArgs{Attn: true, AttnEvent: "permission.updated", AttnCwd: "/g/y", AttnKind: "opencode"}, false},
		{"event missing value", []string{"--attn", "--event"}, ParsedArgs{}, true},
		{"cwd missing value", []string{"--attn", "--cwd"}, ParsedArgs{}, true},
